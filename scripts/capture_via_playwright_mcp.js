...
class TheSolverCapture {
    ...
    async setupDirectories() {
        const dirs = [
            'assets', 'html_raw', 'dom_trees', 
            'screenshots', 'har', 'mirror'
        ];
        ...
        try {
            ...
            const domData = await this.page.evaluate(() => {
                return {
                    ...
                    playerTableData: Array.from(document.querySelectorAll('table tr')).slice(1).map(row => {
...
`;
        ...
        await fs.writeFile(
            path.join(this.captureDir, 'components.md'), 
            componentsMarkdown
        );
    }
...
`;
        ...
        await fs.writeFile(
            path.join(this.captureDir, 'readme.md'), 
            readme
        );
    }
}
