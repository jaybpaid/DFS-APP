...
def display_ai_analysis_results(results: Dict, sport: str):
    ...
    if "optimization" in analyses:
        ...
        print("\n🎯 OPTIMIZATION STRATEGY:")
        print(f"  Core Players: {', '.join(opt['optimal_lineup']['core_players'])}")
        print(f"  Value Plays: {', '.join(opt['optimal_lineup']['value_plays'])}")
        print(f"  Total Salary: ${opt['optimal_lineup']['total_salary']:,} | Projected: {opt['optimal_lineup']['projected_points']:.1f} pts")
        print(f"  AI Insights: {opt['ai_insights']['strategy_recommendation']}")

    # ROI Analysis
    if "roi" in analyses:
        roi = analyses["roi"]
        print("\n💰 ROI ANALYSIS:")
        print(f"  Conservative Strategy: {roi['conservative_strategy'].expected_roi:.1f}% ROI")
        print(f"  Aggressive Strategy: {roi['aggressive_strategy'].expected_roi:.1f}% ROI")
        print(f"  Balanced Strategy: {roi['balanced_strategy'].expected_roi:.1f}% ROI")
        print(f"  Risk-Adjusted ROI: {roi['risk_adjusted_analysis']['risk_adjusted_return']:.1f}%")
        print(f"  Bankroll Allocation: {roi['recommended_bankroll_allocation']['total_recommended']*100:.1f}% per entry")

    # Boom/Bust Analysis
    if "boom_bust" in analyses:
        bb = analyses["boom_bust"]
        print("\n📈 BOOM/BUST ANALYSIS:")
        print("  Top Boom Candidates:")
    ...
    if "contrarian" in analyses:
        ...
        print("\n  Over-Owned to Fade:")
        for play in contrarian["over_owned_to_fade"][:2]:
