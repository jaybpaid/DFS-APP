...
function findMainSlate(contests, sport) {
    ...
    const validContests = contests.filter(contest => {
        ...
        const isMainType = contest.gameType === 'Classic' || 
...
function validatePool(players, sport) {
    ...
    if (players.length < minSize) {
        throw new Error(`Player pool too small: ${players.length} < ${minSize} for ${sport}`);
    }
    ...
    console.log(`✅ Validation passed: ${players.length} ${sport} players, ${positions.size} position types`);
}
...
function generateReport(results) {
    ...
    results.forEach(result => {
        if (result.success) {
            console.log(`✅ ${result.sport}: ${result.playerCount} players (${result.filename})`);
        } else {
