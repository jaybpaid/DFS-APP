#!/bin/bash
# PR1 RELIABLE FILESYSTEM MCP SHIM
...
ensure_container_running() {
    ...
    echo "🚀 Starting hardened filesystem MCP container..."
    cd "$PROJECT_ROOT"

    docker-compose -f docker-compose.hardened-mcp.yml up -d filesystem-hardened
...
main() {
    echo "🔧 PR1 RELIABLE FILESYSTEM MCP SHIM STARTING"

    # Attempt to use hardened containerized MCP first
    if ensure_container_running; then
        echo "🎯 Using hardened containerized Filesystem MCP"
        # Execute MCP server via docker exec with STDIO
        exec docker exec -i "$CONTAINER_NAME" node /app/mcp-server.js filesystem
    else
        ...
        if command -v node >/dev/null 2>&1; then
            ...
            local local_mcp_paths=(
                ...
                "$PROJECT_ROOT/mcp-servers/filesystem.js"
                "$PROJECT_ROOT/mcp/filesystem/index.js"
                "$PROJECT_ROOT/apps/api/mcp/filesystem.js"
            )

            for mcp_path in "${local_mcp_paths[@]}"; do
                if [[ -f "$mcp_path" ]]; then
                    echo "📂 Using local Filesystem MCP: $mcp_path"
                    export NODE_ENV=development
                    export MCP_SERVER=filesystem
                    exec node "$mcp_path"
                fi
            done
        fi
        ...
        echo "❌ CRITICAL: No filesystem MCP server available - neither containerized nor local"
        echo "💡 Solutions:"
        echo "   1. Ensure docker-compose.hardened-mcp.yml is properly configured"
        echo "   2. Install Node.js and MCP packages locally"
