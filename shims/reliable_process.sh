...
ensure_container_running() {
    ...
    echo "🚀 Starting hardened process MCP container..."
    cd "$PROJECT_ROOT"

    if ! docker-compose -f docker-compose.hardened-mcp.yml up -d process-hardened; then
        echo "❌ Failed to start process-hardened container"
        return 1
    fi
...
main() {
    echo "🔧 PR1 RELIABLE PROCESS MCP SHIM STARTING"

    # Attempt to use hardened containerized MCP first
    if ensure_container_running; then
        echo "🎯 Using hardened containerized Process MCP"
        # Execute MCP server via docker exec with STDIO
        exec docker exec -i "$CONTAINER_NAME" node /app/mcp-server.js process
    else
        ...
        if command -v node >/dev/null 2>&1; then
            ...
            local local_mcp_paths=(
                "$PROJECT_ROOT/node_modules/@modelcontextprotocol/server-everything/dist/index.js"
                "$PROJECT_ROOT/mcp-servers/process.js"
                "$PROJECT_ROOT/mcp/process/index.js"
                "$PROJECT_ROOT/apps/api/mcp/process.js"
            )

            for mcp_path in "${local_mcp_paths[@]}"; do
                if [[ -f "$mcp_path" ]]; then
                    echo "📂 Using local Process MCP: $mcp_path"
                    export NODE_ENV=development
                    export MCP_SERVER=process
                    exec node "$mcp_path"
                fi
            done
        fi
        ...
        echo "❌ CRITICAL: No process MCP server available - neither containerized nor local"
        echo "💡 Solutions:"
        echo "   1. Ensure docker-compose.hardened-mcp.yml is properly configured"
        echo "   2. Install Node.js and MCP process server packages locally"
