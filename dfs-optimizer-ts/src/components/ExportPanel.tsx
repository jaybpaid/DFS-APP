    ...
    const text = lineups
      .map((lineup, index) => {
        return (
          ...
          lineup.players
            .map(
              p => `  ${p.positions.join('/')} - ${p.name} (${p.team}) - $${p.salary}`
            )
            .join('\n') +
      ...
      {lineups.length > 0 && (
        <div>
          ...
          <div className='text-sm space-y-1'>
            ...
            <div>
              Average Projection:{' '}
              {lineups.reduce((sum, l) => sum + l.projectedScore, 0) / lineups.length}
            </div>
            <div>
              Unique Players:{' '}
              {new Set(lineups.flatMap(l => l.players.map(p => p.playerId))).size}
            </div>
          </div>
        </div>
      )}
