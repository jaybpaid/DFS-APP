...
const PlayerPool: React.FC<PlayerPoolProps> = ({ players, onPlayersUpdate, settings }) => {
  const [searchTerm, setSearchTerm] = useState('');
  const [filterPosition, setFilterPosition] = useState('');

  const filteredPlayers = players.filter(player => {
    const matchesSearch = player.name.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesPosition = filterPosition === '' || player.positions.includes(filterPosition);
    return matchesSearch && matchesPosition;
  });

  const handlePlayerToggle = (playerId: string, selected: boolean) => {
    const updatedPlayers = players.map(player =>
      player.playerId === playerId ? { ...player, status: selected ? 'active' : undefined } : player
    );
              ...
              <tr key={player.playerId} className="hover:bg-gray-50">
                <td className="p-2 border">
                  <input
                    ...
                    onChange={(e) => handlePlayerToggle(player.playerId, e.target.checked)}
                  />
                </td>
